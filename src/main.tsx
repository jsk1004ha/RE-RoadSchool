    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
        <button onClick={() => navigate('/')} className="flex items-center gap-2 text-left">
          <span className="grid h-10 w-10 place-items-center rounded-2xl bg-blue-50 text-blue-600"><Route size={22} /></span>
          ...
          {!regionConfirmed ? (
            ...
            <>
              ...
              <Card className="p-7 md:p-10">
                ...
                <div className="mt-8 flex justify-between"><Button variant="secondary" onClick={() => setIndex(Math.max(0, index - 1))}>이전</Button>{index === surveyQuestions.length - 1 ? <button type="button" onClick={complete} disabled={!allAnswered} className={classNames('btn btn-primary', !allAnswered && 'cursor-not-allowed opacity-55 hover:translate-y-0 hover:shadow-none')}>AI 분석 시작하기 <Sparkles size={18} /></button> : <Button onClick={() => setIndex(Math.min(surveyQuestions.length - 1, index + 1))}>다음 <ArrowRight size={18} /></Button>}</div>
              </Card>
            </>
          )}
        </motion.div>
...
function RoadmapPage() {
  const months = ['상태 정리 및 상담 연결', '검정고시 기초학습 시작', '모의고사 및 진로상담', '시험 준비 또는 대안교육 탐색', '학력 취득 이후 진로 선택', '대학, 직업훈련, 취업, 복학 중 선택'];
  const checks = ['상담 예약하기', '검정고시 일정 확인하기', '취약 과목 선택하기', '지원기관 방문하기', '가족 또는 상담자와 계획 공유하기'];
  return <DashboardShell active="/roadmap"><div className="dashboard-content"><h1 className="text-3xl font-black text-slate-950">나의 추천 경로</h1><p className="mt-2 font-semibold text-slate-500">검정고시 준비 → 심리상담 병행 → 진로탐색 → 학습계획 확정</p><Card className="mt-6 p-6"><div className="grid gap-4 md:grid-cols-6">{months.map((month, index) => <div key={month} className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-100"><span className="text-sm font-black text-blue-600">{index + 1}개월차</span><p className="mt-2 text-sm font-bold leading-6 text-slate-700">{month}</p></div>)}</div></Card><Card className="mt-5 p-6"><h2 className="text-xl font-black">체크리스트</h2><div className="mt-4 grid gap-3 md:grid-cols-2">{checks.map((check) => <label key={check} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-4 font-bold text-slate-700"><input type="checkbox" className="h-5 w-5 accent-blue-600" /> {check}</label>)}</div></Card></div></DashboardShell>;
}
