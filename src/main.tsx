...
function RoadmapPage() {
  ...
  const roadmap = buildRoadmapModel(result);
  return <DashboardShell active="/roadmap"><div className="dashboard-content"><section className="relative overflow-hidden rounded-[32px] bg-gradient-to-br from-blue-600 via-indigo-600 to-cyan-500 p-8 text-white shadow-2xl"><div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-white/15" /><div className="absolute -bottom-20 left-1/3 h-56 w-56 rounded-full bg-white/10" /><span className="rounded-full bg-white/15 px-4 py-2 text-sm font-black">{result.finalType} 맞춤 경로 구조도</span><h1 className="relative mt-5 max-w-3xl text-4xl font-black tracking-[-0.04em]">{roadmap.headline}</h1><p className="relative mt-4 max-w-3xl text-lg font-bold leading-8 text-blue-50">{roadmap.summary}</p></section><Card className="relative mt-6 overflow-hidden p-6"><div className="roadmap-map"><div className="roadmap-center"><span className="text-3xl">🌱</span><strong>{roadmap.centerLabel}</strong><small>{roadmap.centerDescription}</small></div>{roadmap.paths.map((path, index) => <motion.div key={path.title} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={classNames('roadmap-node', `roadmap-node-${path.tone}`, index === 0 && 'roadmap-node-primary')}><div className="flex items-center justify-between gap-3"><span className="text-3xl">{path.icon}</span><span className="rounded-full bg-white/70 px-3 py-1 text-xs font-black text-slate-600">{path.priorityLabel}</span></div><h2 className="mt-4 text-xl font-black text-slate-950">{path.title}</h2><p className="mt-2 text-sm font-bold leading-6 text-slate-600">{path.text}</p><ol className="mt-4 space-y-2 text-sm font-black text-slate-700">{path.steps.map((step) => <li key={step} className="rounded-2xl bg-white/75 px-3 py-2">{step}</li>)}</ol></motion.div>)}</div></Card><div className="mt-6 grid gap-5 xl:grid-cols-[.9fr_1.1fr]"><Card className="p-6"><h2 className="text-2xl font-black text-slate-950">이번 주 체크포인트</h2><p className="mt-2 font-semibold text-slate-500">큰 결정을 바로 내리지 말고, 결과에 맞춰 작게 확인해보는 순서입니다.</p><div className="mt-5 grid gap-3">{roadmap.checkpoints.map((check) => <label key={check} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-4 font-bold text-slate-700"><input type="checkbox" className="h-5 w-5 accent-blue-600" /> {check}</label>)}</div></Card><Card className="p-6"><h2 className="text-2xl font-black text-slate-950">상담자와 비교할 질문</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{roadmap.questions.map((question) => <div key={question} className="rounded-3xl border border-blue-100 bg-blue-50/60 p-4 font-bold leading-7 text-slate-700">{question}</div>)}</div><Button className="mt-5" onClick={() => navigate('/report')}>상담 리포트와 함께 보기 <ArrowRight size={17} /></Button></Card></div></div></DashboardShell>;
}
